...
log using eventdd_sf49ers, text replace
drop if year>=2023
replace new_stad=0 if mover==1
replace move_stad=. if new_stad==0
replace mover=1 if team_id==428
replace new_stad=0 if team_id==428
replace move_year=move_stad if team_id==428
replace move_stad=. if team_id==428
...
putexcel set table_sf_results, replace
