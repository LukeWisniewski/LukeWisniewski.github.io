...
/*
foreach i in revenue avg_attendance revexpratio salary wpct ticket_price{
	use "$directory/Dta_Files/complete_data.dta", replace
	collapse (mean) mean_`i'=`i' (semean) se_`i'=`i', by(year league)
	gen ci_lower_`i'=mean_`i'-1.96*se_`i'
	gen ci_upper_`i'=mean_`i'+1.96*se_`i'
	drop if year==2020
	
	foreach j in 1 2 3 4{
	twoway (line mean_`i' year if league==`j', ytitle("") xtitle("Year")), xscale(range(1990 2022)) xlabel(1990(5)2022)
	graph export means_`i'_`j'.jpg, replace
	}
}
*/
